...
│   └── server/
│       ├── api/              # HTTP 路由 / SSE 事件 / 中间件
│       ├── context/          # 上下文窗口 + 记忆系统（FIFO/摘要/Core Memory/被动注入）
│       ├── core_memory/      # persona 人设
│       ├── dispatch/         # 工具调度器
│       ├── langgraph/        # 编排：Supervisor 意图路由 + Evolution 状态机
│       ├── llm/              # LLM Provider 适配
│       ├── mcp/tools/        # 50+ MCP 工具（检索/标注/演化/对比/地图）
│       ├── observability/    # 三层日志 + 全链路 trace + 指标
│       ├── prompts/          # Prompt 模板（YAML）
│       ├── rag/              # RAG：五路召回 + boost 重排 + 标签打标
...
│   └── src/
│       ├── components/       # agent / charts / compare / evolution / map / textreader
│       ├── stores/           # Pinia 状态
│       ├── views/            # 页面视图
│       ├── composables/      # SSE / 地图 / 状态组合式函数
│       └── services/         # API 与 SSE 事件总线
│
├── server/                   # 业务后端（Node.js + Fastify）
│   └── src/
│       ├── routes/           # agent / annotations / auth / literature / locations
│       ├── services/         # 文献 / 分段 / 用户数据
│       └── middleware/       # 鉴权
│
