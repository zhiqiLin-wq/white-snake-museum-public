...
收集四类原始值（persons / locations / plot_units / topics），每类交给 LLM 归并：
合并后的标准名 -> canonical，原始叫法 -> raw，生成词表供阶段 5 query 打标。
"""
import asyncio
import json
import re
...
def collect():
    data = json.load(open(RESULTS / "chunk_labels.json", encoding="utf-8"))
...
async def main():
    ...
    for kind, merged in results:
        ...
        print(f"[DONE] {kind}: {n_entries} 个词条（待删除 {n_del}）", flush=True)

    out_path = RESULTS / "merged_vocab2.json"
